...
tell application "Google Chrome"
  ...
  set printReady to execute targetTab javascript "(() => { const text = document.body ? document.body.innerText : ''; if (!text || text.length < 40) return 'waiting'; const host = location.hostname; const path = location.pathname; if (host === 'github.com' && (path.endsWith('/pulls') || path.endsWith('/pulls/'))) { const hasCounts = text.includes('Open') && text.includes('Closed'); const hasRows = Array.from(document.links || []).some(function(a) { return a.href.includes('/pull/'); }); const hasEmptyState = text.includes('No results matched') || text.includes('Welcome to pull requests!') || (text.includes('0 Open') && text.includes('0 Closed')); return (hasCounts && (hasRows || hasEmptyState)) || hasEmptyState ? 'ready' : 'waiting'; } if (host === 'github.com' && path.includes('/commits')) { const hasCommitText = text.includes('Commits'); const hasSha = /[0-9a-f]{7}/.test(text); const hasEmptyState = /No commits|No results|This branch has no commits/i.test(text); return hasCommitText && (hasSha || hasEmptyState) ? 'ready' : 'waiting'; } return 'ready'; })()"
